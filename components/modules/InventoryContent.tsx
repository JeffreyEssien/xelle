    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                    <h1 className="text-3xl font-serif text-brand-dark">Inventory Management</h1>
                    <div className="flex gap-4 mt-2 text-sm text-gray-600">
                        <div>
                            <span className="block text-xs uppercase tracking-wider text-gray-400">Total Value (Cost)</span>
                            <span className="font-mono font-bold text-brand-dark">{formatCurrency(totalInventoryValue)}</span>
                        </div>
                        <div>
                            <span className="block text-xs uppercase tracking-wider text-gray-400">Potential Profit</span>
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <div className="flex gap-2 bg-brand-lilac/10 p-1 rounded-md">
                        <button
                            onClick={() => setTab("stock")}
                            className={`px-4 py-1.5 rounded-sm text-sm font-medium transition-colors ${tab === "stock" ? "bg-white shadow-sm text-brand-purple" : "text-brand-dark/60 hover:text-brand-purple"}`}
                        >
                            Stock & Pricing
                        </button>
                        <button
                            onClick={() => setTab("history")}
                            className={`px-4 py-1.5 rounded-sm text-sm font-medium transition-colors ${tab === "history" ? "bg-white shadow-sm text-brand-purple" : "text-brand-dark/60 hover:text-brand-purple"}`}
                        >
                            History Log
                        </button>
                    </div>
            ...
            {isCreating && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 overflow-y-auto max-h-[90vh]">
                        <h2 className="text-xl font-serif text-brand-dark mb-4">Add New Inventory Item</h2>
                        <form onSubmit={handleCreate} className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Name</label>
                                    <input required type="text" value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">SKU (Optional)</label>
                                    <input type="text" value={newItem.sku} onChange={e => setNewItem({ ...newItem, sku: e.target.value })} placeholder="Auto-generated if empty" className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Cost Price ($)</label>
                                    <input required type="number" value={newItem.costPrice} onChange={e => setNewItem({ ...newItem, costPrice: e.target.value })} className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Selling Price ($)</label>
                                    <input required type="number" value={newItem.sellingPrice} onChange={e => setNewItem({ ...newItem, sellingPrice: e.target.value })} className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Initial Stock</label>
                                    <input required type="number" value={newItem.stock} onChange={e => setNewItem({ ...newItem, stock: e.target.value })} className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Reorder Level</label>
                                    <input required type="number" value={newItem.reorderLevel} onChange={e => setNewItem({ ...newItem, reorderLevel: e.target.value })} className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                                <div className="sm:col-span-2">
                                    <label className="block text-xs font-medium text-gray-500 mb-1">Supplier (Optional)</label>
                                    <input type="text" value={newItem.supplier} onChange={e => setNewItem({ ...newItem, supplier: e.target.value })} className="w-full px-3 py-2 border border-brand-lilac/30 rounded focus:border-brand-purple" />
                                </div>
                            </div>
            ...
            <input
                ...
                className="w-full sm:w-80 px-4 py-2 border border-brand-lilac/30 rounded-sm focus:outline-none focus:border-brand-purple"
            />
            ...
            {isAdjusting && selectedItem && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                        <h2 className="text-xl font-serif text-brand-dark mb-4">Manage: {selectedItem.name}</h2>
                        <form onSubmit={handleUpdate} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <div className="w-20 px-3 py-2 bg-gray-100 rounded text-center font-mono text-gray-600" title="Current Stock">
                                        {selectedItem.stock}
                                    </div>
                                    <input
                                        type="number"
                                        ...
                                        placeholder="+ / -"
                                        className="flex-1 px-4 py-2 border border-brand-lilac/30 rounded-sm focus:outline-none focus:border-brand-purple"
                                    />
                                </div>
                                <p className="text-xs text-gray-500 mt-1">Leave empty if only updating price.</p>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2 border border-brand-lilac/30 rounded-sm focus:outline-none focus:border-brand-purple"
                                />
                            </div>

                            <div className="flex justify-end gap-3 pt-4 border-t border-brand-lilac/10 mt-6">
